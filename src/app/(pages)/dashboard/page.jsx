...
export default function DashboardPage() {
  ...
  const planPoints = { annual: 100, monthly: 50, representative: 0 };
  const planPrice = { annual: "$720 / Year", monthly: "$60 / Month", representative: "Ask a Rep" };
  ...
  // 🔥 Load Member & calculate points dynamically
  useEffect(() => {
    let unsub;
    const unsubAuth = onAuthStateChanged(auth, async (user) => {
      ...
      unsub = onSnapshot(ref, async (snap) => {
        ...
        // calculate points: referral count * points per plan
        const pointsPerReferral = planPoints[data.plan] || 0;
        const newPoints = (data.referrals?.length || 0) * pointsPerReferral;

        // Update Firestore if points changed
